    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
            <div className="text-center space-y-2">
                ...
                <p className="text-muted-foreground">各ステップの評価結果を比較します。</p>
                <p className="text-sm text-muted-foreground">発行済株式数: {totalShares.toLocaleString()}株</p>
            </div>
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b-2 border-primary">
                                    <th className="text-left py-4 px-4 font-bold text-lg">項目</th>
                                    <th className="text-center py-4 px-4">
                                        <div className="font-bold text-lg text-primary">相続税評価額</div>
                                        <div className="text-xs font-normal text-muted-foreground mt-1">Step 6</div>
                                    </th>
                                    <th className="text-center py-4 px-4">
                                        <div className="font-bold text-lg text-green-700">法人税法上の時価</div>
                                        <div className="text-xs font-normal text-muted-foreground mt-1">Step 7</div>
                                    </th>
                                    <th className="text-center py-4 px-4">
                                        <div className="font-bold text-lg text-amber-700">シミュレーション</div>
                                        <div className="text-xs font-normal text-muted-foreground mt-1">Step 8（直前期利益=0）</div>
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr className="bg-blue-50 border-b border-slate-200">
                                    <td className="py-4 px-4 font-bold text-base">評価額（1株）</td>
                                    <td className="text-center py-4 px-4">
                                        <div className="text-2xl font-bold text-primary">{results.step6.finalValue.toLocaleString()}</div>
                                        <div className="text-xs text-muted-foreground">円</div>
                                    </td>
                                    <td className="text-center py-4 px-4">
                                        <div className="text-2xl font-bold text-green-700">{results.step7.finalValue.toLocaleString()}</div>
                                        <div className="text-xs text-muted-foreground">円</div>
                                    </td>
                                    <td className="text-center py-4 px-4">
                                        <div className="text-2xl font-bold text-amber-700">{results.step8.finalValue.toLocaleString()}</div>
                                        <div className="text-xs text-muted-foreground">円</div>
                                    </td>
                                </tr>
                                <tr className="bg-white border-b border-slate-200">
                                    <td className="py-4 px-4 font-bold text-base">評価額（総額）</td>
                                    <td className="text-center py-4 px-4">
                                        <div className="text-xl font-bold text-primary">{(results.step6.finalValue * totalShares).toLocaleString()}</div>
                                        <div className="text-xs text-muted-foreground">円</div>
                                    </td>
                                    <td className="text-center py-4 px-4">
                                        <div className="text-xl font-bold text-green-700">{(results.step7.finalValue * totalShares).toLocaleString()}</div>
                                        <div className="text-xs text-muted-foreground">円</div>
                                    </td>
                                    <td className="text-center py-4 px-4">
                                        <div className="text-xl font-bold text-amber-700">{(results.step8.finalValue * totalShares).toLocaleString()}</div>
                                        <div className="text-xs text-muted-foreground">円</div>
                                    </td>
                                </tr>
                                <tr className="bg-blue-50 border-b border-slate-200">
                                    ...
                                    <td className="text-center py-4 px-4">
                                        <div className="text-lg font-semibold">{results.step6.comparableValue.toLocaleString()}</div>
                                        <div className="text-xs text-muted-foreground">円</div>
                                    </td>
                                    <td className="text-center py-4 px-4">
                                        <div className="text-lg font-semibold">{results.step7.comparableValue.toLocaleString()}</div>
                                        <div className="text-xs text-muted-foreground">円</div>
                                    </td>
                                    <td className="text-center py-4 px-4">
                                        <div className="text-lg font-semibold">{results.step8.comparableValue.toLocaleString()}</div>
                                        <div className="text-xs text-muted-foreground">円</div>
                                    </td>
                                </tr>
                                <tr className="bg-white border-b border-slate-200">
                                    ...
                                    <td className="text-center py-4 px-4">
                                        <div className="text-lg font-semibold">{results.step7.netAssetPerShare.toLocaleString()}</div>
                                        <div className="text-xs text-muted-foreground">円</div>
                                    </td>
                                    <td className="text-center py-4 px-4">
