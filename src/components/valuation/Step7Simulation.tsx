    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Result Comparison */}
                <Card className={`col-span-1 md:col-span-2 p-8 border-4 shadow-xl bg-gradient-to-br from-white to-primary/5 ${simulationResults.diff < 0 ? 'border-primary/20' : 'border-secondary/20'}`}>
                    <div className="flex flex-col md:flex-row justify-around items-center gap-8">
                        {/* Current */}
                        <div className="text-center space-y-2 flex-1">
                            <h3 className="text-sm font-bold text-muted-foreground">現状の評価額</h3>
                            <div className="text-4xl font-black text-foreground">
                                {simulationResults.current.finalValue.toLocaleString()}
                                <span className="text-lg text-muted-foreground ml-1">円</span>
                            </div>
                            <p className="text-xs text-muted-foreground">{simulationResults.current.methodDescription}</p>
                        </div>
                        ...
                        {/* Simulation */}
                        <div className="text-center space-y-2 flex-1">
                            <h3 className="text-sm font-bold text-primary">直前期利益=0 の場合</h3>
                            <div className="text-4xl font-black text-primary">
                                {simulationResults.simulated.finalValue.toLocaleString()}
                                <span className="text-lg text-muted-foreground ml-1">円</span>
                            </div>
                            <p className="text-xs text-muted-foreground">{simulationResults.simulated.methodDescription}</p>
                        </div>
                    </div>

                    {/* Diff Display */}
                    <div className="mt-8 text-center bg-white/50 p-4 rounded-xl border border-dashed">
                        <span className="text-sm font-bold text-muted-foreground mr-4">差額</span>
                        <span className={`text-2xl font-black ${simulationResults.diff < 0 ? 'text-blue-600' : 'text-red-600'}`}>
                            {simulationResults.diff > 0 ? '+' : ''}{simulationResults.diff.toLocaleString()} 円
                        </span>
                    </div>
                </Card>
                ...
                <Card className="p-6 border-secondary/20">
                    ...
                    <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                            ...
                            <span className="font-bold">{simulationResults.current.comparableValue.toLocaleString()} 円</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-muted-foreground">純資産価額</span>
                            <span className="font-bold">{simulationResults.current.netAssetPerShare.toLocaleString()} 円</span>
                        </div>
                    </div>
                </Card>

                <Card className="p-6 border-primary/20">
                    <h4 className="font-bold border-b pb-2 mb-4 text-primary">直前期利益=0 (詳細)</h4>
                    <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                            <span className="text-muted-foreground">類似業種比準価額</span>
                            <span className="font-bold">{simulationResults.simulated.comparableValue.toLocaleString()} 円</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-muted-foreground">純資産価額</span>
                            <span className="font-bold">{simulationResults.simulated.netAssetPerShare.toLocaleString()} 円</span>
                        </div>
                    </div>
                </Card>
